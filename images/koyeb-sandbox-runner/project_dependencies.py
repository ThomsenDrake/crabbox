...
def capture_dependencies(root, policy, cache=None):
    ...
    removed = [path for path in removed if not any(path.startswith(parent + "/") for parent in removed if parent != path)]
...
def restore_dependencies(stage, recipes, entries, cache=None):
    ...
    return {path: entry for path, entry in baseline.items()
            if not any(path == removed or path.startswith(removed + "/") for removed in recipe["removedPaths"])}
